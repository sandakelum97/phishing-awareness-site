...
const appController = {
    init() {
        ...
        if (pagePath === 'login.html' || pagePath === '') {
            this.handleLoginPage();
        } else if (pagePath.includes('_quiz.html')) {
            this.handleQuizPage();
        } else if (pagePath === 'results.html') {
            this.handleResultsPage();
        } else if (pagePath === 'certificate.html') {
            this.handleCertificatePage();
        } else { // Hub page
            this.handleHubPage();
        }
    },
    ...
    // Utility Functions
    checkLogin() {
        const userData = localStorage.getItem('chesmiUserData');
        if (!userData) {
...
const quizController = {
    ...
    showResults() {
        ...
        certContainer.innerHTML = ''; // Clear previous buttons
        ...
        if (score >= 9) {
            ...
            scoreMessage = `A score of 9 or higher is required to pass. Please retake the quiz. 💪`;
            bodyClass = 'results-fail';
            particleConf = particleConfig.danger;
            hubBtn.style.display = 'none'; // Hide hub button if failed
        }
    ...
    updateSecurityVisuals() {
        ...
        appController.runParticles(config);
    },
